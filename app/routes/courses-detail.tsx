...
import { Star, Users, Clock, Play, Lock, Unlock, ChevronDown } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          <div className="h-48 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center">
          ...
          <Card><CardHeader><CardTitle>강좌 소개</CardTitle></CardHeader><CardContent><div className="whitespace-pre-wrap">{c.description}</div></CardContent></Card>
                  ...
                  <AccordionContent>
                    <div className="space-y-1">
                      {lessons.filter(l => l.chapterId === ch.id).map((l) => (
                        <div key={l.id} className="flex items-center justify-between p-2 rounded hover:bg-gray-50">
                          <div className="flex items-center gap-2">
                            {l.isFree || enrollment ? <Play className="h-4 w-4 text-blue-500" /> : <Lock className="h-4 w-4 text-gray-400" />}
                            <span className="text-sm">{l.title}</span>
                            {l.isFree && <Badge variant="outline" className="text-xs text-blue-500">미리보기</Badge>}
                          </div>
        ...
        <div className="space-y-4">
          <Card className="sticky top-20">
            <CardContent className="p-6">
              ...
              {enrollment ? (
                <Button asChild className="w-full bg-blue-600"><Link to={`/courses/${c.id}/learn`}>학습 계속하기</Link></Button>
              ) : currentUser ? (
                <form method="post"><Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700">{c.price === 0 ? '무료로 시작하기' : '수강하기'}</Button></form>
              ) : (
                <Button asChild className="w-full bg-blue-600"><Link to="/login">로그인 후 수강</Link></Button>
              )}
            </CardContent>
          </Card>
          {instructor && (
            <Card>
              <CardHeader><CardTitle className="text-base">강사 정보</CardTitle></CardHeader>
              <CardContent>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-semibold">{(instructor.name || '?')[0]}</div>
                  <div><div className="font-medium">{instructor.name}</div>{instructor.bio && <p className="text-sm text-gray-500 line-clamp-2">{instructor.bio}</p>}</div>
                </div>
              </CardContent>
            </Card>
          )}
