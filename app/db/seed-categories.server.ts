...
export function seedKmongCategories(db: Database.Database) {
  const findStmt = db.prepare('SELECT id FROM categories WHERE slug = ?');
  const insertStmt = db.prepare('INSERT INTO categories (name, slug, icon, sortOrder) VALUES (?, ?, ?, ?)');
